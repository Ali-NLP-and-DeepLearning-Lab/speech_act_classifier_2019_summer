...
class BiLSTM_Enc_RAM(nn.Module):
    ...
    def init_hidden_pos(self,batch_size):
        return (torch.randn(2*self.num_layers, batch_size, self.hidden_dim_pos // 2).to(self.device),
                torch.randn(2*self.num_layers, batch_size,self.hidden_dim_pos // 2).to(self.device))

    def init_hidden_att(self,batch_size):
        return (torch.randn(2, batch_size, self.hidden_dim_att // 2).to(self.device),
                torch.randn(2, batch_size,self.hidden_dim_att // 2).to(self.device))

    def init_hidden(self,batch_size):
        return (torch.randn(2*self.num_layers, batch_size, self.hidden_dim_word // 2).to(self.device),
                torch.randn(2*self.num_layers, batch_size,self.hidden_dim_word // 2).to(self.device))
...
class BiLSTM_Dec_CR2(nn.Module):
    ...
    def init_hidden_logit(self,batch_size):
        return (torch.randn(2*self.num_layers, batch_size, self.num_tag).to(self.device),
                torch.randn(2*self.num_layers, batch_size,self.num_tag).to(self.device))
